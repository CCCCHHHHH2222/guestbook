...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="hack0242/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/CCCCHHHHH2222/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        ...
        success { 
            slackSend(tokenCredentialId: 'slack-token4'
                , channel: '#교육'
                , color: 'good'
                , message: "hh의 ${JOB_NAME} (${BUILD_NUMBER}) 빌드가 성공적으로 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        failure { 
            slackSend(tokenCredentialId: 'slack-token4'
                , channel: '#교육'
                , color: 'danger'
                , message: "hh의 ${JOB_NAME} (${BUILD_NUMBER}) 빌드가 실패하였습니다. Details: (<${BUILD_URL} | here >)")
    }
  }
}
